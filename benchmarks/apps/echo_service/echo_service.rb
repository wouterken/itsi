...
class EchoServiceImpl < Echo::EchoService::Service
  ...
  def echo_collect(requests, _call)
    messages = []
    count = 0
  ...
  def process_payment(request, _call)
    ...
    status = if request.amount.units > 0
