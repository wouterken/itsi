...
* **No isolated testing environment**: Benchmarks were run on personal development machines without measures to isolate CPU, memory, or disk I/O from background processes. This may introduce variability or minor skew in some results.
...
### gRPC
The server bundled with the Ruby [gRPC gem](https://github.com/grpc/grpc/tree/master/src/ruby) is the de facto standard for gRPC services in the Ruby ecosystem. However, it doesn't utilize a Fiber scheduler and therefore was more prone to thread exhaustion errors, particularly for streaming calls, than Itsi.
