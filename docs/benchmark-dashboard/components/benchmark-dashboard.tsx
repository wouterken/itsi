...
import { InfoIcon, TrendingUp, TrendingDown, TagIcon } from "lucide-react"
...
export function BenchmarkDashboard({ data }: BenchmarkDashboardProps) {
  ...
  const allFilterOptions: FilterOptions = useMemo(() => {
    ...
    return {
      ...
      testCases: [...new Set(flattenedData.map((item) => item.test_case).filter(Boolean))],
      servers: [...new Set(flattenedData.map((item) => item.server).filter(Boolean))],
      threads: [...new Set(flattenedData.map((item) => item.threads).filter((t) => typeof t === "number"))].sort(
        (a, b) => a - b,
      ),
      workers: [...new Set(flattenedData.map((item) => item.workers).filter((w) => typeof w === "number"))].sort(
        (a, b) => a - b,
      ),
      concurrencyLevels: [
        ...new Set(flattenedData.map((item) => item.concurrency).filter((c) => typeof c === "number")),
      ].sort((a, b) => a - b),
      http2Options: ["all", ...new Set(flattenedData.map((item) => item.http2).filter((h) => typeof h === "boolean"))],
    }
  }, [data, flattenedData])
  ...
  const serverColors = useMemo(() => {
    ...
    return allFilterOptions.servers.reduce(
      (acc, server, index) => {
        acc[server] = colors[index % colors.length]
        return acc
      },
  ...
  const parseUrlParams = useCallback((): Partial<FilterState> | null => {
    ...
    try {
      ...
      if (visibleServersParam) {
        try {
          filters.visibleServers = visibleServersParam.split(",").filter(Boolean)
        } catch (e) {
  ...
  const updateUrlParams = useCallback((filters: FilterState) => {
    ...
    try {
      ...
      if (filters.visibleServers && filters.visibleServers.length > 0) {
        url.searchParams.set("visibleServers", filters.visibleServers.join(","))
      } else {
  ...
  const validateAndSanitizeFilters = useCallback(
    (urlFilters: Partial<FilterState>): FilterState => {
      ...
      const validatedFilters: FilterState = {
        cpu: allFilterOptions.cpus.includes(urlFilters.cpu || "") ? urlFilters.cpu! : defaultFilters.cpu,
        ...
        concurrency: allFilterOptions.concurrencyLevels.includes(urlFilters.concurrency || 0)
          ? urlFilters.concurrency!
          : defaultFilters.concurrency,
        http2: allFilterOptions.http2Options.includes(urlFilters.http2 ?? "all")
          ? urlFilters.http2!
          : defaultFilters.http2,
        xAxis: ["concurrency", "threads", "workers"].includes(urlFilters.xAxis || "")
          ? urlFilters.xAxis!
          : defaultFilters.xAxis,
        metric: ["rps", "p95_latency", "errorRate"].includes(urlFilters.metric || "")
          ? urlFilters.metric!
          : defaultFilters.metric,
        visibleServers: Array.isArray(urlFilters.visibleServers)
          ? urlFilters.visibleServers.filter((server) => allFilterOptions.servers.includes(server))
  ...
  const [visibleServers, setVisibleServers] = useState<Record<string, boolean>>(() => {
    ...
    const urlFilters = parseUrlParams()
    if (urlFilters?.visibleServers && Array.isArray(urlFilters.visibleServers)) {
      allFilterOptions.servers.forEach((server) => {
        initialVisibleServers[server] = urlFilters.visibleServers!.includes(server)
      })
    }
  ...
  const [hoveredPoint, setHoveredPoint] = useState<BenchmarkResult | null>(null)
  ...
  const filterOptions = useMemo(() => {
    // First filter by CPU
    const cpuFilteredData = flattenedData.filter((item) => (item as any).cpu === filters.cpu)

    // Get available test cases for the selected CPU
    const availableTestCases = [...new Set(cpuFilteredData.map((item) => item.test_case).filter(Boolean))]

    // Then filter by test case to get the remaining filter options
    const testCaseFilteredData = cpuFilteredData.filter((item) => item.test_case === filters.testCase)

    return {
      cpus: allFilterOptions.cpus,
      testCases: availableTestCases,
      servers: [...new Set(testCaseFilteredData.map((item) => item.server).filter(Boolean))],
      threads: [...new Set(testCaseFilteredData.map((item) => item.threads).filter((t) => typeof t === "number"))].sort(
        (a, b) => a - b,
      ),
      workers: [...new Set(testCaseFilteredData.map((item) => item.workers).filter((w) => typeof w === "number"))].sort(
        (a, b) => a - b,
      ),
      concurrencyLevels: [
        ...new Set(testCaseFilteredData.map((item) => item.concurrency).filter((c) => typeof c === "number")),
      ].sort((a, b) => a - b),
      http2Options: [
        "all",
        ...new Set(testCaseFilteredData.map((item) => item.http2).filter((h) => typeof h === "boolean")),
      ],
    }
  ...
  useEffect(() => {
    setFilters((prev) => {
      ...
      if (prev.xAxis !== "threads" && !filterOptions.threads.includes(prev.threads)) {
        newFilters.threads = filterOptions.threads[0] || 1
      }

      if (prev.xAxis !== "workers" && !filterOptions.workers.includes(prev.workers)) {
        newFilters.workers = filterOptions.workers[0] || 1
      }

      if (prev.xAxis !== "concurrency" && !filterOptions.concurrencyLevels.includes(prev.concurrency)) {
  ...
  const filteredData = useMemo(() => {
    return flattenedData.filter((item) => {
      ...
      if (filters.xAxis !== "threads" && item.threads !== filters.threads) return false
      if (filters.xAxis !== "workers" && item.workers !== filters.workers) return false
      if (filters.xAxis !== "concurrency" && item.concurrency !== filters.concurrency) return false
    ...
    switch (filters.metric) {
      ...
      case "p95_latency":
        return {
          ...
          isBetter: "lower",
          icon: TrendingDown,
          formatter: (value: number) => (value >= 1000 ? formatCompact(value, 1) : value.toFixed(2)),
        }
      case "errorRate":
        return {
          label: "Error Rate",
  ...
  const visibleDataKeys = useMemo(() => {
    ...
    filterOptions.servers.forEach((server) => {
      if (visibleServers[server]) {
        if (filters.http2 === "all") {
          ...
          const serverData = filteredData.filter((item) => item.server === server)
    ...
    return keys
  }, [filterOptions.servers, visibleServers, filters.http2, filteredData, formatServerName])
  ...
  const visibleServersForUrl = useMemo(() => {
    return Object.keys(visibleServers).filter((server) => visibleServers[server])
  }, [visibleServers])
            ...
            <CardContent className="p-2">
              <div className="space-y-2">
                ...
                <div className="space-y-1">
                  ...
                  <Select value={filters.cpu} onValueChange={(value) => handleFilterChange("cpu", value)}>
                ...
                <div className="space-y-1">
                  ...
                  <Select value={filters.testCase} onValueChange={(value) => handleFilterChange("testCase", value)}>
                    <SelectTrigger id="test-case-filter" className="h-7 text-xs">
                      <SelectValue placeholder="Select test" />
                    </SelectTrigger>
                    <SelectContent>
                      {groupedTestCases.sortedGroups.map((group) => (
                        <SelectGroup key={group}>
                          <SelectLabel className="text-xs font-medium text-muted-foreground pl-2">{group}</SelectLabel>
                          {groupedTestCases.groupedTests[group].map((testCase) => (
                            <SelectItem key={testCase} value={testCase} className="text-xs pl-6">
                              {testCase}
                            </SelectItem>
                          ))}
                        </SelectGroup>
                ...
                <div className="grid grid-cols-2 gap-2">
                  ...
                  <div className="space-y-1">
                    ...
                    <Select
                      ...
                      onValueChange={(value) => handleFilterChange("threads", Number.parseInt(value))}
                      disabled={filters.xAxis === "threads"}
                    >
                      <SelectTrigger id="threads-filter" className="h-7 text-xs">
                        <SelectValue placeholder={filters.xAxis === "threads" ? "On X-Axis" : "Select threads"} />
                      </SelectTrigger>
                      <SelectContent>
                        {filterOptions.threads.length > 0 ? (
                          filterOptions.threads.map((thread) => (
                            <SelectItem key={thread} value={thread.toString()} className="text-xs">
                              {thread}
                            </SelectItem>
                          ))
                        ) : (
                  ...
                  <div className="space-y-1">
                    ...
                    <Select
                      ...
                      onValueChange={(value) => handleFilterChange("workers", Number.parseInt(value))}
                      disabled={filters.xAxis === "workers"}
                    >
                      <SelectTrigger id="workers-filter" className="h-7 text-xs">
                        <SelectValue placeholder={filters.xAxis === "workers" ? "On X-Axis" : "Select workers"} />
                      </SelectTrigger>
                      <SelectContent>
                        {filterOptions.workers.length > 0 ? (
                          filterOptions.workers.map((worker) => (
                            <SelectItem key={worker} value={worker.toString()} className="text-xs">
                              {worker}
                            </SelectItem>
                          ))
                        ) : (
                ...
                <div className="grid grid-cols-2 gap-2">
                  ...
                  <div className="space-y-1">
                    ...
                    <Select
                      ...
                      onValueChange={(value) => handleFilterChange("concurrency", Number.parseInt(value))}
                      disabled={filters.xAxis === "concurrency"}
                    >
                      <SelectTrigger id="concurrency-filter" className="h-7 text-xs">
                        <SelectValue
                          placeholder={filters.xAxis === "concurrency" ? "On X-Axis" : "Select concurrency"}
                        />
                      </SelectTrigger>
                      <SelectContent>
                ...
                <div className="space-y-1">
                  ...
                  <Select value={filters.xAxis} onValueChange={(value) => handleFilterChange("xAxis", value)}>
                ...
                <div className="space-y-1">
                  ...
                  >
                    <TabsList className="h-6 w-full">
                      <TabsTrigger value="rps" className="text-xs px-2 py-0 h-4 flex-1">
                        RPS
                      </TabsTrigger>
                      <TabsTrigger value="p95_latency" className="text-xs px-2 py-0 h-4 flex-1">
                        P95
                      </TabsTrigger>
                      <TabsTrigger value="errorRate" className="text-xs px-2 py-0 h-4 flex-1">
                        Errors
                      </TabsTrigger>
                    </TabsList>
                  </Tabs>
                ...
                {summaryStats && (
                  <div className="pt-1 space-y-1 text-xs">
                    ...
                    <div className="flex justify-between">
                      ...
                      <span className="font-medium">{metricInfo.formatter(summaryStats.min)}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-muted-foreground">Max:</span>
                      <span className="font-medium">{metricInfo.formatter(summaryStats.max)}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-muted-foreground">Avg:</span>
                      <span className="font-medium">{metricInfo.formatter(summaryStats.avg)}</span>
                    </div>
                  </div>
                )}
              </div>
              ...
              <CardHeader className="pb-0 pt-2 px-3">
                <CardTitle className="text-base">
                  ...
                  <span className="text-xs font-normal ml-2 text-muted-foreground">
                    {filters.xAxis !== "threads" && `Threads: ${filters.threads}, `}
                    {filters.xAxis !== "workers" && `Workers: ${filters.workers}, `}
                    {filters.xAxis !== "concurrency" && `Concurrency: ${filters.concurrency}, `}
                      ...
                      <ResponsiveContainer width="100%" height="100%">
                        ...
                        >
                          ...
                          <XAxis
                            ...
                            label={{
                              value: filters.xAxis.charAt(0).toUpperCase() + filters.xAxis.slice(1),
                          ...
                          {visibleDataKeys.map((dataKey, index) => (
                            <Bar
                              ...
                              name={dataKey}
                              fill={
                                serverColors[dataKey.split(" ")[0].replace(/\+/g, "__")] ||
                                serverColors[Object.keys(serverColors)[index % Object.keys(serverColors).length]]
                              }
                              ...
                              onMouseOver={(data) =>
                                handleChartHover({
                                  activePayload: [{ dataKey, payload: data.payload }],
                                })
                              }
                              onMouseLeave={() => {
                                setHoveredPoint(null)
                      ...
                      {filterOptions.servers.map((server) => (
                        <div
                          ...
                          className={`flex items-center gap-1 px-2 py-0.5 rounded cursor-pointer transition-opacity ${
                            visibleServers[server] ? "opacity-100" : "opacity-40"
                          }`}
                        ...
                        >
                          <div className="w-2 h-2 rounded-sm" style={{ backgroundColor: serverColors[server] }} />
                          <span className="text-xs">{formatServerName(server)}</span>
                        </div>
                      ))}
                    </div>
                  </>
            ...
            <Card className="shadow-sm">
              ...
              <CardContent className="p-2">
                {hoveredPoint ? (
                  <div className="space-y-2">
                    ...
                    <div className="flex flex-wrap items-center gap-2">
                      <Badge variant="outline" className="flex items-center">
                        <div
                          ...
                          style={{ backgroundColor: serverColors[hoveredPoint.server] }}
                        />
                        {formatServerName(hoveredPoint.server)}
                      </Badge>
                      <Badge variant="outline">{hoveredPoint.test_case}</Badge>
                      <Badge variant="outline">
                        {filters.xAxis}: {hoveredPoint[filters.xAxis as keyof BenchmarkResult]}
                      </Badge>
                      <Badge variant="outline">{hoveredPoint.http2 ? "HTTP/2" : "HTTP/1.1"}</Badge>
                    </div>
                    ...
                    {hoveredPoint.version && (
                      <div className="flex items-center text-xs text-muted-foreground">
                        ...
                        <span className="font-medium overflow-hidden text-ellipsis">{hoveredPoint.version}</span>
                      </div>
                    )}
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-x-4 gap-y-1 text-xs">
                      <div className="flex justify-between">
                        ...
                        <span className="font-medium">{hoveredPoint.results.requestsPerSec.toFixed(2)}</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-muted-foreground">Success Rate:</span>
                        <span className={`font-medium ${hoveredPoint.results.successRate < 1 ? "text-red-600" : ""}`}>
                          {(hoveredPoint.results.successRate * 100).toFixed(2)}%
                        </span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-muted-foreground">P95 Latency:</span>
                        <span className="font-medium">{hoveredPoint.results.p95_latency.toFixed(4)} ms</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-muted-foreground">Avg Latency:</span>
                        <span className="font-medium">{hoveredPoint.results.average.toFixed(4)} ms</span>
                      </div>
                    </div>

                    {/* Error distribution */}
                    {Object.keys(hoveredPoint.results.errorDistribution || {}).length > 0 ? (
                      <div>
                        <div className="text-xs font-medium mb-1">Error Distribution:</div>
                        <div className="space-y-1">
                  ...
                  <p className="text-xs text-muted-foreground">Hover over a data point to see benchmark details</p>
                )}
              </CardContent>
            </Card>
          </div>
